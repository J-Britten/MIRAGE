...
with adjustments made to better support the needs for Unity inpainting:
- Removed uint8 input and output requirements as Unity uses float32 for colors
- We exclusively use float32 tensors for all operations
- We remove all operations where we multiply or divide by 255 in pre- and post-processing
- We remove clipping preprocessing as it is done in Unity already (and we need the bounds to properly stitch the image back together)
- Change output so we get the direct output from the model. this allows us to do mask postprocessing in unity
"""
...
class MIGAN_Pipeline(nn.Module):
    ...
    def get_masked_bbox(self, mask):
        ...
        xx = mask.mean(dim=0)
        yy = mask.mean(dim=1)
        xx_masked_ids = w_indices[xx < torch.tensor(1.0)]#255.0)]
        yy_masked_ids = h_indices[yy < torch.tensor(1.0)]#255.0)]
    ...
    def postprocess(self, image, mask, model_output):
        ...
        model_output = ((model_output * 0.5 + 0.5)).clamp(0, 1)
       ...
       ## composed_img = image * mask + model_output * (1 - mask)
        composed_img = model_output
        #composed_img = model_output * (1 - mask)
        #return composed_img.clamp(0, 255).to(torch.uint8)
        return composed_img.clamp(0.0, 1.0)#.to(torch.uint8)
    ...
    def forward(self, image: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        #x_min, x_max, y_min, y_max = self.get_masked_bbox(mask.to(torch.uint8))
...
def main():
    ...
    dummy_image = torch.ones(1, 3, 512, 512, device="cpu", dtype=torch.float32)# * 255
    dummy_mask = torch.ones(1, 1, 512, 512, device="cpu", dtype=torch.float32)# * 255
    dummy_mask[:, :, 10, 10] = 0
    dummy_mask[:, :, 20, 30] = 0
    input_names = ["image", "mask"]
    output_names = ["result"]
    (args.output_dir / "models").mkdir(parents=True, exist_ok=True)
    onnx_export_path = args.output_dir / "models" / "migan_input.onnx"
    torch.onnx.export(
        migan_pipeline,
